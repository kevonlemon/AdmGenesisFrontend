...
import { INICIO } from './admnomina/inicio';
import { PARAMETROS } from './admnomina/parametros';
import { PROCESOS } from './admnomina/procesos';
import { REPORTES } from './admnomina/reportes';
...
export default function Router() {
  ...
  rutas = typeof rutas === "undefined" ? [] : rutas
  rutas = ordenarListaJson(rutas, 'menu', 'asc');
  componentes.forEach(c => {
    rutas.forEach(r => {
      if (r.url === c.url) {
        rutasistema.push(
          { path: `${r.menu?.toLocaleLowerCase()}${r.url}`.trim(), element: c.element }
        )
      }
    })
  })
  ...
  return useRoutes([
    ...
    // Dashboard Routes
    {
      ...
      children: [
        ...
        // /* SISTEMA */
        // { path: "ajusteprecio", element: <AjustePrecio /> },
