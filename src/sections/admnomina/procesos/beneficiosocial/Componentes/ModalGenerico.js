...
export default function ModalPacientes(props) {
  ...
  const item = props.busquedaTipo[1].tipo;
  ...
  const filterTable = (buscar) => {
    ...
    const result = dataOrigin.filter((e) => e.nombre.includes(buscar) || e.item.includes(buscar));
    setRowFilter(result);
  };
          ...
          >
            ...
            >
              ...
              sx={estilosdatagrid}
                ...
                disableColumnFilter
                rows={rowsFilter}
                getRowId={(rowsFilter) => rowsFilter.item}
              />
            </div>
          </Box>
        </Box>
