...
export default function Beneficiosocial() {
  ...
  useEffect(() => {
    async function getseteo() {
      const response = await axios(`${URLAPIGENERAL}/empleados/listar`, config);
      const dataRes = response.data;
  ...
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const [tiposBusquedasProducto, setTiposBusquedaProducto] = useState([
    { tipo: 'nombre' },
    { tipo: 'codigo' },
    { tipo: 'cedula' },
  ]);
  ...
  const validarsave = () => {
    ...
    const empleadodesde = Datospaciente.codigo;
    const empleadohasta = Datospaciente2.codigo;

    if (!esfechaValida) {
      mensajeGenerico('error', 'Por favor, debe colocar algun año anterior o el año actual');
      // setError(true);
      setFechaerror(true);
      return false;
    }

    if (empleadodesde === '---' || empleadodesde.trim() === '') {
      mensajeGenerico('error', 'Por favor, debe seleccionar un empleado inicial');
      setError1(true);
      return false;
    }

    if (empleadohasta === '---' || empleadohasta.trim() === '') {
      mensajeGenerico('error', 'Por favor, debe seleccionar un rango de empleado');
      setError2(true);
      return false;
    }
  ...
  const BUSCAR = async () => {
    ...
    try {
      ...
      const response = await axios(
        `${URLAPIGENERAL}/beneficioempleado/listar?Empleado1=${Datospaciente.id}&Empleado2=${Datospaciente2.id}`,
        config,
        setMostrarProgreso(true)
      );
      ...
      if (response.status === 200) {
        if (datos.length > 0) {
          ...
          // mensajeGenerico('succes', 'Busqueda completada con exito');

          setRows(listagrupada);
        }

        if (datos.length <= 0) {
          mensajeGenerico('error', 'No existen datos relacionados');
      ...
      if (error.response.status === 500) {
        mensajeGenerico('error', 'Debe colocar correctamente los rangos de empleado');
      }
      if (error.response.status !== 500) {
  ...
  const Grabar = async () => {
    try {
      ...
      const response = await axios.post(
        `${URLAPIGENERAL}/beneficioempleado/grabar`,
        listaempleado,
        config,
        setMostrarProgreso(true)
      );
      ...
      if (response.status === 200) {
        mensajeGenerico('succes', 'Datos guardados con éxito');
      }
    } catch (error) {
      // console.log(error.response)
      if (error.response.status === 400) {
        mensajeGenerico('error', 'Ya existen registros almacenados con estas referencias');
      }
      if (error.response.status !== 400) {
        mensajeGenerico('error', 'Error en el servidor');
      }
  ...
  async function buscarEmpleados(empleado) {
    if (empleado === 'desde') {
      if (Datospaciente.codigo === '---' || Datospaciente.codigo.trim() === '') {
        console.log('llega?');
        setOpenModallistpacientes(true);
      } else {
        try {
          const { data } = await axios(
            `${URLAPIGENERAL}/empleados/obtenerxcodigo?codigo=${
              Datospaciente.codigo === '' ? 'string' : Datospaciente.codigo
            }`,
            config
          );
          if (data.length === 0) {
      ...
      if (Datospaciente2.codigo === '---' || Datospaciente2.codigo.trim() === '') {
        setOpenModalProducto(true);
      } else {
        try {
          const { data } = await axios(
            `${URLAPIGENERAL}/empleados/obtenerxcodigo?codigo=${
              Datospaciente2.codigo === '' ? 'string' : Datospaciente2.codigo
            }`,
            config
          );
          if (data.length === 0) {
            setNoExisteProducto(true);
            mensajeGenerico('Código no encontrado', 'warning');
          } else {
            setDatospaciente2({
              ...Datospaciente2,
              id: data.codigo,
                            ...
                            <Grid item md={3} xs={12} sm={3}>
                              <LocalizationProvider dateAdapter={AdapterDateFns} locale={es}>
                                <MobileDatePicker
                                  ...
                                  renderInput={(params) => (
                                    <TextField {...params} error={error} fullWidth size="small" />
                                  )}
                                />
                    ...
                    >
                      <DataGrid
                        ...
                        getCellClassName={(params) => {
                          if (params.value == null || params.row.codigo === '') {
                            return '';
                          }
